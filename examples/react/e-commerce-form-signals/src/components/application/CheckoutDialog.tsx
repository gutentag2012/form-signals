...
function CartList() {
  const subtotal = useComputed(() =>
    productsField.data.value.reduce(
      (acc, item) =>
        acc + item.data.value.price.value * item.data.value.quantity.value,
      0,
    ),
  )
  const shipping = 5.99
    ...
    <productsField.FieldProvider>
      <div className="flex flex-col gap-2">
        <div className="flex flex-col gap-1">
          {!productsField.data.value.length && (
            ...
            <div key={item.key} className="flex flex-row items-center gap-2">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm dark:text-gray-400">
                  ${item.data.value.price.value.toFixed(2)}
                </p>
              </div>
...
function ShippingAddressForm() {
  const shippingAddressForm = useFieldGroup(form, [
    'name',
    'email',
    'address',
    'city',
