...
interface FileInputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, "value" | "onChange" | "type"> {
  label?: string
  useBound?: boolean
  preview?: "img" | "list"
}

export function FileInput({ className, label, useBound, preview="list", ...props }: FileInputProps) {
  ...
  const previewDisplay = !data ? null : preview === "list" ? (<>
          ...
          {data.filter(Boolean).map((file) => (
            "key" in file ? <li key={file.key}>{file.data.value.name}</li> : <li key={file.name}>{file.name}</li>
          ))}
          </ul>
          ...
          {data.filter(Boolean).map((file) => (
            "key" in file ? <img
              ...
              alt="Prview" className="max-w-4xl object-cover"
            /> : <img
              key={file.name}
      ...
      />
      {
        data ? (previewDisplay) : (<>
          ...
          <div className="flex flex-col items-center">
            <p className='font-semibold text-foreground/80 text-sm'>Click to upload or drag over</p>
