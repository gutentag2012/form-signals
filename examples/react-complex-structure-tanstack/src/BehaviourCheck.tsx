...
export const BehaviourCheck = () => {
  ...
  const form = useForm<{name: string, other: string, nested: {deep: string}}>({
    validators: {
      onSubmit: v => {
  ...
  useEffect(() => {
    const timeout1 = setTimeout(() => form.setFieldValue("other", "value"), 1000)
    const timeout2 = setTimeout(() => form.setFieldValue("other", "value2"), 2000)
    const timeout3 = setTimeout(() => form.fieldInfo?.other?.instances?.[0]?.handleChange("value3"), 3000)
  ...
  return (
    <main className="container mt-3">
      <h1 className="text-4xl font-extrabold tracking-tight mb-1">BehaviourCheck</h1>
      <form.Provider>
        {show && <form.Field
          name={"other"}
          preserveValue
