...
export class FieldApi<TValue, TName extends Paths<TValue>> {
	...
	private readonly _isDirty: ReadonlySignal<boolean> = computed(() => {
		...
		const currentValueStringValues = JSON.stringify(
			unSignalifyValue(this.signal.value),
		);
	...
	public async validateForEvent(
		...
		const asyncValidationPromises = validators.async.map(async (validator) => {
			...
			if (!validator.debounceMs) {
				return validator.validate(value, abortController.signal).then((res) => {
					errors[validator.key] = res;
				});
			}
			...
			return new Promise<string | undefined>((resolve) => {
				setTimeout(async () => {
					...
					await validator
						.validate(value, abortController.signal)
						.then((res) => {
							errors[validator.key] = res;
						});
					resolve(undefined);
	 ...
	 */
	public handleChange(newValue: ValueAtPath<TValue, TName>) {
	...
	public swapValuesInArray<IndexA extends number, IndexB extends number>(
		...
		indexA: ValueAtPath<TValue, TName> extends any[]
			? number
			: // biome-ignore lint/suspicious/noExplicitAny: This could be any array
			  ValueAtPath<TValue, TName> extends readonly any[]
			  ? ValueAtPath<TValue, TName>[IndexA] extends ValueAtPath<
						TValue,
						TName
				  >[IndexB]
					? number
			  ...
			  : never,
		// biome-ignore lint/suspicious/noExplicitAny: This could be any array
		indexB: ValueAtPath<TValue, TName> extends any[]
			? number
			: // biome-ignore lint/suspicious/noExplicitAny: This could be any array
			  ValueAtPath<TValue, TName> extends readonly any[]
			  ? ValueAtPath<TValue, TName>[IndexB] extends ValueAtPath<
						TValue,
						TName
				  >[IndexA]
					? number
